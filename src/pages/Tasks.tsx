...
const Tasks: React.FC = () => {
  const [tasks, setTasks] = useState([
    ...
    { id: 2, text: 'Read chapter 5', due: '2023-10-26', completed: true },
  ]);
      ...
      <IonContent fullscreen className="content-background">
        <div className="welcome-content">
          ...
          <IonGrid className="ion-no-padding">
            <IonRow>
              <IonCol>
                <IonButton expand="block" fill="outline" className="btn-round" onClick={handleCancel}>
                  Cancel
                </IonButton>
              </IonCol>
              <IonCol>
                <IonButton expand="block" className="btn-round2" onClick={handleAddTask}>
                  Add Task
                </IonButton>
              </IonCol>
            </IonRow>
          ...
          <div className="completed-list">
            {tasks.filter(t => t.completed).map((task) => (
              <div className="completed-item" key={task.id} onClick={() => toggleComplete(task.id)}>
             ...
             {tasks.filter(t => t.completed).length === 0 && (
               <p style={{textAlign: 'center', color: '#888'}}>No completed tasks yet.</p>
            )}
          </div>
